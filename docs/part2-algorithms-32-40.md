...
<div class="section-intro">本节建立图像处理与检测任务之间的联系，关注算法步骤的目的、输入条件、参数影响和结果评价。</div>

## 32. 什么是手眼标定（Hand-Eye Calibration）？Eye-In-Hand 和 Eye-To-Hand 的区别是什么？ { #q32 }
...
## 33. 什么是模板匹配？NCC、SAD、形状匹配的原理和适用场景有何不同？ { #q33 }
...
## 34. 什么是 Blob 分析？如何用它完成连通域计算、面积筛选和工业缺陷定位？ { #q34 }
...
## 35. 图像预处理的标准工具链是什么？滤波、形态学操作各解决什么问题？ { #q35 }
...
## 36. 什么是 OCR 和 OCV？如何检测字符、条码、二维码的可读性和正确性？ { #q36 }
...
## 37. 什么是 MTF（调制传递函数）？如何用它评价镜头和相机系统的真实解像力？ { #q37 }
...
## 38. 什么是无监督异常检测（Anomaly Detection）？在没有缺陷样本的情况下如何训练工业检测模型？ { #q38 }
...
## 39. 什么是双目立体视觉的视差（Disparity）？如何从视差图计算深度？误差来源有哪些？ { #q39 }
...
## 40. 视觉系统项目验收时，客户通常提出哪些标准？如何设计完整的量产前验收测试方案？ { #q40 }
