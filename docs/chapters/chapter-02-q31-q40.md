...
对每个测试特征，计算它到 Memory Bank 中最近特征的欧氏距离，并把这个距离作为异常分数。正常测试 patch 的平均最近邻距离约为 0.1528，异常测试 patch 约为 2.5241；按排名计算得到的 AUROC 约为 0.9982。若取两类均值的中点 1.3385 作为演示阈值，示例精确率为 1.0、召回率为 0.8267。这个阈值只是为了展示混淆矩阵含义，**不是原稿推荐的生产阈值。**
...
按工程中常见的 AIAG 风格参考区间，GR&R 小于 10% 通常可接受，10%～30% 需要结合用途判断，大于 30% 通常不可接受。这个演示结果 15.0% 属于条件可接受区间，且操作员间再现性略大于重复性，排查重点应放在夹具、操作流程和定位一致性，而不是先怀疑相机或算法。这里的样件数量和阈值用于解释方法，不是本题原稿规定的唯一验收方案。
...
[返回总目录](../index.md) · [上一章](chapter-01-q01-q30.md) · [下一章](chapter-03-q41-q50.md)
