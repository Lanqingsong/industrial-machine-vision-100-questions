# 第二章 算法基础与系统验收（Q31–Q40）

> 所属模块：模块二：算法基础与系统验收（Q31–Q40）

<a id="q31"></a>
## 第31问：拿到一个新的检测项目，你的硬件选型（相机、镜头、光源）的具体步骤是什么？
---
...
工业视觉硬件选型，是根据检测任务为**相机、镜头和光源确定一套能够稳定成像的配置**。相机负责把光学图像转换成数字图像；镜头决定视场、放大关系和清晰范围；光源通过照明方向、波段和亮度改变缺陷与背景的对比度。
...
这里的**视场（FOV，Field of View）**，指一次图像能够覆盖的被测物区域尺寸；**最小特征尺寸**，指希望系统稳定看见的最小结构或缺陷；**允许误差**，指测量结果可以接受的偏差。**触发**是用传感器、编码器或控制信号确定采图时刻；**飞拍**则是在目标运动过程中完成采图，通常还要同时处理运动模糊问题。
...
相机分辨率通常从视场与最小可分辨特征的关系推导。若沿某一方向的视场大小为 \(FOV\)，希望该方向上最小特征尺寸为 \(d_{\min}\)，并且要求这个特征至少占据 \(n\) 个像素，则理论像素数可写为
...
更稳妥的流程是先按式（31-1）算出理论下限，再检查是否存在以下放大因素：视野是否还要预留边界、目标姿态是否会变化、是否需要后续裁切、是否存在多工位共用一台相机、以及缺陷是否位于视野边缘。**理论像素数只是起点，最后选定的相机分辨率还应与镜头分辨能力、帧率需求和接口带宽一起核算；高像素并不会自动换来高可检性。**
...
光谱选择也应服务于目标特征。黑白相机配单色 LED 是最常见的组合，红光稳定、寿命长、成本合适，因此使用广泛；蓝光波长更短，对某些微小表面缺陷和细节边缘更敏感，但对杂散反射和表面状态也更挑剔；若目标与背景是颜色差异而非灰度差异，就应把彩色成像或分光方案一并纳入考虑。光源颜色不能凭经验固定，很多项目要靠样件实拍比较红、蓝、白、近红外等不同波段的实际效果。若现场环境光复杂，在镜头前增加与照明波段匹配的窄带滤光片，往往能显著提高稳定性。

#### 31.6 硬件组件之间如何匹配和集成？

相机、镜头和光源构成一套相互牵制的成像链。相机像元越小，对镜头分辨能力和照明质量的要求通常越高；镜头焦距越长、光圈收得越小，光源亮度和安装空间压力往往越大；光源角度一改，镜头看到的反射路径和对比度分布也会跟着变。硬件选型必须把这些因素放在一起评估。
...
![新的视觉项目硬件选型流程](images/q31-q40/part7_31_01_hardware_selection_flow.png)


<p align="center"><strong>图31-1 新的视觉项目硬件选型流程</strong></p>
...
#### 31.8.1 计算示例：从特征尺寸、相机分辨率到镜头焦距

31.2 和 31.4 中的两个公式应当配合使用。下面用同一组参数演示完整计算过程：视场为 120 mm，需要分辨的最小特征为 0.30 mm，工作距离为 300 mm，传感器在对应方向上的有效宽度为 11.3 mm。

由式（31-1）可知，同一个物理特征，因任务类型不同而需要不同的像素数。存在性检测取 (n=2.5)，水平方向理论上需要约 1000 像素；形状、方向或边缘定位取 (n=4)，需要约 1600 像素；尺寸测量取 (n=6)，需要约 2400 像素。再将工作距离、视场和传感器宽度代入式（31-2），可得匹配焦距约为 28 mm。这个结果还要结合镜头实际靶面覆盖范围、像元尺寸下的分辨能力和现场景深进行确认。



![相机分辨率与镜头焦距计算示例](images/q31-q40/part7_31_02_resolution_lens_worked_example_zh.png)


<p align="center"><strong>图31-2 相机分辨率与匹配镜头焦距的计算示例</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">
图31-2 的左图比较了三类任务对相机水平分辨率的要求：同一最小特征尺寸下，尺寸测量需要的余量最大。右图进一步说明，特征尺寸越小，任务类型之间的像素需求差距越明显。因此，“最小缺陷尺寸 ÷ 像素当量”只能得到采样起点，不能代替镜头分辨率、边缘对比度和测量重复性的综合验证。
</div>

### 常见误区

**把最小缺陷尺寸直接换算成像素数，却没有区分检出任务和测量任务。** 存在性判断、形状或边缘定位、尺寸测量对应不同的像素覆盖要求，使用同一个安全系数会造成分辨率预算浪费，或者让特征处于欠采样状态。

**相机和镜头分别选型，不检查镜头是否真的能解析到传感器像元对应的细节。** 高像素传感器配上分辨率不足的镜头，只会更密地采样模糊图像。

**尚未确认运动状态，就提前指定卷帘快门相机。** 如果项目需要飞拍，快门方式应在硬件选型初期确定；等系统成型后再补救，成本和风险都会明显增加。

**把实验室里一次拍清楚当成产线可交付。** 传送带振动、杂散光反射、班次温度变化和连续运行时的缓存压力，往往只有在接近真实工况的测试中才会暴露。

### 术语与符号表

**安全系数 (n)**：式（31-1）中的像素覆盖倍数，表示目标特征至少应跨越多少个像素。它随任务类型变化，不是所有项目都固定取同一个数。

**镜头靶面（image circle）**：镜头能够形成可用图像的圆形区域。靶面不能完整覆盖传感器时，图像边缘可能出现渐晕或清晰度下降。

**法兰距**：镜头接口的机械基准面到传感器平面的固定距离。法兰距不匹配时，即使镜头本身没有问题，也可能无法正确合焦。

**节拍与稳定性验证**：在实际产线或等效工位上，检查系统能否在连续速度、振动和温度变化下保持成像与通信稳定的阶段。它验证的是系统能否长期工作，而不是单张图像是否好看。

**CMOS/CCD**：两类图像传感器技术。CMOS 通常具有较高集成度和较低功耗；CCD 由电荷转移方式读出，历史上具有较好的均匀性，但现代工业相机中 CMOS 更常见。

**GigE、USB3、Camera Link、CoaXPress**：常见工业相机接口。它们在带宽、传输距离、延迟确定性、布线和采集卡要求上不同，应结合相机数量与帧率选择。

### 延伸阅读

- 第2问给出了由缺陷尺寸和视场估算最低分辨率的基础推导；本题进一步强调任务类型对安全系数的影响。
- 第7问讨论焦距、光圈、工作距离和视场之间的关系，第9问进一步说明远心镜头的适用场景。
- 第40问将本题的现场验证方法扩展为正式的量产前验收方案。
...
<a id="q32"></a>
## 第32问：什么是手眼标定（Hand-Eye Calibration）？Eye-In-Hand 和 Eye-To-Hand 的区别是什么？
---
...
抓取点通常还要叠加工具坐标变换、法向约束和安全接近路径。像素坐标到机器人动作之间，实际对应的是一整条由成像、空间变换和运动规划构成的链路。



![Eye-In-Hand 与 Eye-To-Hand 两种手眼标定构型示意](images/q31-q40/part7_32_01_hand_eye_layouts.png)
...
#### 32.8.1 计算示例：为什么手眼求解不能只满足数学最小样本数

式（32-4）给出的数学最小条件是 3 组不共面位姿，但只满足这个下限时，解很容易退化。图32-2用简化仿真比较了两种采集策略：一类位姿具有多样化的旋转轴，另一类位姿的旋转轴几乎相同，接近“小角度绕同一轴旋转”的退化情况。

在恰好 3 组位姿时，近似共线旋转轴的策略在本示例中的最坏情况旋转误差约为 90°，结果基本无法使用；旋转轴分布充分多样的策略在同样的样本数下，误差已经低于约 2°。随着位姿数量增加到工程上常用的 10 组以上，两种策略的中位误差和离散程度都会下降，但多样化位姿始终明显更稳定。**位姿的空间多样性与数量同样重要，不能把“求解器返回了结果”当成标定可靠的证据。**



![位姿数量和位姿多样性对手眼标定误差的影响](images/q31-q40/part7_32_02_pose_count_worked_example_zh.png)


<p align="center"><strong>图32-2 位姿数量和旋转轴多样性对手眼求解误差的影响</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">图32-2把“位姿数量”和“位姿是否重复”分开比较。三组位姿虽然达到数学下限，但若旋转轴接近同一直线，噪声会被放大成很大的姿态误差；增加数量并扩大旋转方向覆盖范围，才能让 AX=XB 的求解具有工程稳定性。图中仿真用于说明敏感性趋势，不等同于某个具体库的生产级求解器实现。</div>

### 常见误区

**把手眼标定和相机内参标定混为一谈。** 内参描述相机如何把三维点投影到传感器上；手眼标定求的是相机坐标系与机器人坐标系之间的刚体变换，解决的是两套坐标系如何对齐的问题。

**只采集数学最小数量的位姿，或所有位姿的旋转变化过小。** 求解器可能仍然返回一个结果，但小测量噪声会被放大成几十度的旋转误差，导致抓取和装配位置不稳定。

**只用重投影误差判断标定质量。** 重投影误差只能说明视觉链对标定数据的拟合程度，不能直接代表末端执行器的定位精度；后者还受手眼变换、机器人重复精度和完整坐标变换链影响。

**标定效果不好时先怀疑算法，而不检查机械和流程条件。** 支架刚性、靶标安装、负载下的机器人重复性以及图像与机器人位姿是否同步，往往比求解算法名称更先决定结果。

### 术语与符号表

**手眼标定**：求取相机坐标系与机器人坐标系之间刚体变换 (X) 的过程，使视觉测得的位置和姿态能够转换为机器人可执行的位姿。

**Eye-In-Hand**：相机安装在机器人末端执行器上，随机器人一起运动的构型。

**Eye-To-Hand**：相机固定在工作站外部，观察机器人工作空间的构型。

**重投影误差**：标定特征的模型预测像素位置与实际检测像素位置之间的偏差。它衡量视觉链的自洽性，不等同于末端任务精度。

**末端定位误差**：机器人按视觉结果运动后，末端目标位姿与真实目标位置之间的空间偏差，通常是项目验收更关心的指标。

**内参与外参**：内参描述焦距、主点和畸变等相机内部成像参数；外参描述相机相对于机器人或其他坐标系的旋转和平移。

**OpenCV**：开源计算机视觉软件库。文中的 `calibrateHandEye()`、`stereoCalibrate()` 等函数是其标定工具，不等于标定本身已经自动满足项目精度要求。

### 延伸阅读

- 第52问和第53问继续讨论视觉系统、PLC、运动控制和传输延迟之间的分工，可用于理解机器人系统中的时间同步问题。
- 第11问关于镜头畸变及其测量影响的讨论，与本题的误差来源直接相关，因为未补偿的畸变会同时影响内参和手眼标定结果。
- 第33问继续介绍模板匹配，内容从坐标变换基础转向常用检测算法。
...
<a id="q33"></a>
## 第33问：什么是模板匹配？NCC、SAD、形状匹配的原理和适用场景有何不同？
---
...
OpenCV 中没有与 SAD（绝对差之和）直接等价的标准匹配模式。`cv2.TM_SQDIFF` 计算的是平方差，`cv2.TM_SQDIFF_NORMED` 计算的是归一化平方差；如果确实需要 SAD，应自行计算绝对差之和，或使用明确支持该度量的函数。SAD 公式为 Σ|T(x,y)-I(x+u,y+v)|，SSD 为 Σ[T(x,y)-I(x+u,y+v)]²。
...
![SAD、NCC 与形状匹配的响应曲线对比](images/q31-q40/part7_33_01_template_matching_compare.png)
...
#### 33.8.1 计算示例：NCC 的归一化如何抵消整体亮度偏移

图33-2构造了一幅 32×32 像素的模板，让整幅图像的亮度偏移从 -80 变化到 +80，并在真实匹配位置同时计算 SAD 和 NCC。SAD 对原始像素差求和，整体亮度发生偏移时，所有像素差会一起增大，因此得分随偏移量近似线性增长；NCC 在相关计算前减去各自的均值并进行尺度归一化，对纯粹的整体加性亮度变化基本保持 1.0 的高相似度。

当偏移量达到约 ±60 及更大时，NCC 也会略有下降，原因是像素值被裁剪到 0 或 255，出现了饱和失真。这不是归一化失效，而是图像采集本身丢失了信息。**NCC 能抵消线性的整体亮度变化，却不能消除非均匀阴影、镜面高光和饱和裁剪等真实照明问题。**



![整体亮度变化下 NCC 与 SAD 的响应比较](images/q31-q40/part7_33_02_ncc_vs_sad_worked_example_zh.png)


<p align="center"><strong>图33-2 整体亮度偏移下 NCC 归一化与 SAD 原始像素差的比较</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">图33-2的重点不是证明 NCC 对所有光照问题都有效，而是展示一个明确边界：纯粹的整体加性偏移可被均值归一化抵消，像素裁剪和空间不均匀照明则仍会破坏匹配。现场调试时，应先判断失败属于亮度整体变化、局部反光还是成像饱和，再决定是否更换匹配方法。</div>

### 常见误区

**没有确认照明稳定性就默认使用 SAD 或 SSD。** 这类方法简单、速度快，适合灰度稳定的受控场景；现场光照波动明显时，直接把它作为唯一定位方法容易产生间歇性失配。

**把 NCC 当成解决光照问题的万能方法。** NCC 对纯线性亮度变化较稳，但非均匀阴影、镜面高光和饱和裁剪仍会降低匹配质量。

**为了保险而无限扩大角度和尺度搜索范围。** 搜索范围越宽、采样越密，计算量和误匹配风险越高。应先利用机械结构和工艺先验收窄搜索空间。

**匹配失败后先调评分阈值。** 更合理的顺序是检查模板、成像条件、搜索范围，最后再调阈值；否则容易把成像问题掩盖成参数问题。

### 术语与符号表

**SAD/SSD**：绝对差之和/差的平方和，直接比较模板与候选区域的像素差，速度快但对亮度、对比度和遮挡变化敏感。

**NCC**：归一化互相关，在计算相关性前对模板和搜索窗口做均值、方差归一化，对线性的加性或乘性亮度变化更稳，但计算量更高。

**形状匹配**：依据梯度方向或边缘几何关系匹配目标，而不是直接比较灰度，适合轮廓清晰且存在部分遮挡的场景。

**图像金字塔搜索**：先在降采样图像上粗略搜索，再对少量候选位置在原始分辨率下精确搜索的加速策略。

**ROI**：感兴趣区域（Region of Interest），指模板匹配实际搜索或输出结果的图像范围。合理裁剪 ROI 可以减少误匹配和计算量。

**OpenCV 模板匹配**：OpenCV 提供的滑动窗口相似度计算工具，可选择平方差、相关系数等度量；函数名不代表所有匹配指标都已内置。

### 延伸阅读

- 第34问介绍 Blob 分析，第37问介绍 MTF，二者分别从区域分割和成像分辨能力补充模板匹配的工程边界。
- 第22问对照明方案的评估方法，有助于理解为什么即使使用 NCC，现场光照仍可能造成匹配失败。
- 第92问讨论 Halcon 与 OpenCV 的取舍，可进一步了解 ShapeModel 这类商业形状匹配实现。
...
<a id="q34"></a>
## 第34问：什么是 Blob 分析？如何用它完成连通域计算、面积筛选和工业缺陷定位？
---
...
工程实施时，用户未必需要手写这套算法，但理解其原理有助于判断为何某些细小连接、边界触碰或噪声点会改变最终连通域数量。很多误判在连通域定义阶段就已经发生，后续特征筛选只是把这个结果继续放大。
...
![Blob 分析从灰度图到目标输出的流程示意](images/q31-q40/part7_34_01_connected_components_pipeline.png)


<p align="center"><strong>图34-1 Blob 分析从灰度分割到特征筛选的典型流程</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">
图34-1 依次画出了原始灰度图、阈值分割、连通域提取、几何特征过滤和目标输出五个阶段。结果稳定性取决于每一步对“哪些像素属于同一个目标”的定义是否一致，不能只归因于最后的面积或圆度阈值。若分割把背景误并入目标，后面的面积、孔洞数、长宽比都会被连带污染；若连通性定义与噪声形态不匹配，又可能把一个真实目标拆成多个碎块。Blob 分析不是单个算子的名称，而是一整套以分割质量为前提的流程。它非常适合轮廓、孔洞、缺口、颗粒计数等任务，但在灰度对比弱、粘连严重或纹理本身复杂的场景中，往往需要先补充更可靠的照明和预处理，再谈连通域判定。
</div>

#### 34.8.1 计算示例：4 连通和 8 连通为什么会同时改变 Blob 数量与面积

图34-2构造了一个简单的二值图像：两个 10×10 像素的实心方块只在一个对角点接触，另外放置一个 3×3 的孤立小区域作为尺度参照。采用 4 连通时，两个方块之间没有共享边，因此被识别为面积均为 100 的两个 Blob；采用 8 连通时，对角相邻也算连通，两个方块会合并成面积 200 的一个 Blob。

如果后续面积筛选设置为拒绝面积大于 150 的区域，原本想排除过大的缺陷团块，那么 4 连通结果会保留两个真实目标，而 8 连通结果会把合并后的两个目标误判为一个过大区域。**连通性不是无关紧要的实现选项，它会直接改变后续面积、周长和形状特征。**



![4连通与8连通下 Blob 数量和面积的比较](images/q31-q40/part7_34_02_connectivity_worked_example_zh.png)


<p align="center"><strong>图34-2 同一二值图像在4连通与8连通下得到的 Blob 数量和面积</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">图34-2把对角接触这一容易忽略的情况单独拿出来。4连通得到 3 个区域，面积约为 100、100 和 9；8连通得到 2 个区域，面积约为 200 和 9。若现场出现面积突然变大或 Blob 数量突然减少，应同时检查连通性定义和二值化结果，不能只调面积阈值。</div>

### 常见误区

**没有确认对角相邻区域是否应视为同一目标，就把 8 连通当成默认安全选项。** 连通性改变后，Blob 数量和面积都会变化，后续所有面积、形状阈值也会受到影响。

**面积筛选出现异常时只怀疑阈值，而不检查上游二值化和连通性。** Blob 分析的瓶颈通常在分割稳定性，连通性选择也可能造成与阈值错误相似的误判。

**把所有特征条件压成一条复杂规则。** 更容易维护的做法是先用面积、位置等稳定特征做粗筛，再用圆度、长宽比等敏感特征做细筛；分层处理也更方便定位现场漂移。

**把 Blob 分析用于依赖纹理、上下文或类别语义的目标。** Blob 能描述区域，却不能理解区域代表什么。两个面积和形状相近的 Blob，实际含义可能完全不同。

### 术语与符号表

**Blob（二值连通域）**：二值图像中由前景像素组成、彼此连通并与背景分离的区域。

**4 连通/8 连通**：连通域标记时对像素邻接关系的两种定义。4 连通只把上下左右视为相邻，8 连通还把四个对角方向视为相邻。

**圆形度**：利用区域面积与周长等量比较目标接近圆形程度的形状特征，常用于区分圆孔、圆斑与细长或不规则缺陷。

**大津法（Otsu）**：根据灰度直方图选择使类间方差最大的全局阈值方法。直方图双峰明显时效果较好，存在亮度渐变或局部反光时稳定性会下降。

**阈值分割**：按照灰度或颜色条件把像素分成前景和背景的过程，是 Blob 连通域分析的输入步骤。

### 延伸阅读

- 第33问的模板匹配用于定位已知形状，本题的 Blob 分析则用于描述分割后得到的任意连通区域。
- 第17问和第18问关于明场、暗场及抗反射照明的讨论，有助于理解 Blob 分析为何常常受二值化质量限制。
- 第90问进一步讨论传统算法与深度学习的选择，可延伸理解 Blob 分析的语义边界。
...
<a id="q35"></a>
## 第35问：图像预处理的标准工具链是什么？滤波、形态学操作各解决什么问题？
---
...
![图像预处理各步骤对视觉效果的影响示意](images/q31-q40/part7_35_01_preprocess_tradeoff.png)
...
#### 35.8.1 计算示例：根据噪声类型选择滤波器

图35-2在一幅具有清晰阶跃边缘的合成图像上，分别加入椒盐噪声和高斯随机噪声，再比较高斯滤波和中值滤波相对于干净参考图像的整幅 RMSE。椒盐噪声的结果很明确：中值滤波几乎去除了全部脉冲噪声，RMSE 从 45.2 降到 3.2；高斯滤波只能部分压制噪声，RMSE 约为 14.1，并且边缘更模糊。

对于高斯随机噪声，两种滤波器都能改善结果；在这个平坦区域加硬边缘的测试中，中值滤波的边缘保持特性使它的 RMSE 也更低。但这不意味着中值滤波适合所有真实图像：在带有纹理的工业图像中，中值滤波可能产生块状或斑驳伪影，而高斯滤波的响应更平滑、可预测。**滤波器应根据真实噪声和缺陷图像验证，不能只根据一条经验规则决定。**



![不同噪声下中值滤波与高斯滤波的比较](images/q31-q40/part7_35_02_filter_choice_worked_example_zh.png)


<p align="center"><strong>图35-2 椒盐噪声与高斯噪声下中值滤波和高斯滤波的比较</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">图35-2把滤波器选择落实到噪声类型：脉冲噪声适合用中值滤波，随机噪声通常从高斯滤波开始评估。数值结果来自合成测试，只能说明机制和趋势；真实产线还要检查缺陷边缘是否被抹平、纹理是否被改变以及下游检测结果是否真正改善。</div>

### 常见误区

**把灰度化、去噪或 CLAHE 当作固定必选步骤。** 如果某一步没有改善下游任务的稳定性，就不应因为“标准流程”而保留。

**凭习惯选择滤波器，而不确认噪声类型。** 椒盐噪声使用高斯滤波会留下明显残差，而中值滤波通常更合适；错误的滤波器会带来可测量的性能损失。

**用过度锐化或过度均衡化掩盖真实的成像问题。** 这些操作会同时放大噪声和伪影，不能替代对焦、照明、反射和安装问题的处理。

**预处理链只增不减。** 每一步都应服务于下一步的输入质量，长期运行中应主动删除没有带来收益的步骤。

### 术语与符号表

**顶帽/黑帽变换**：分别提取小尺度亮异常和暗异常的形态学操作，利用原图与开运算或闭运算结果之间的差异去除缓慢变化的背景。

**CLAHE**：限制对比度的自适应直方图均衡化，在局部区域增强对比度，同时限制过度放大，降低全局均衡化造成的过拉伸风险。

**椒盐噪声**：少量像素随机变成极亮或极暗值的脉冲噪声，通常更适合用中值滤波抑制。

**结构元素**：定义腐蚀、膨胀、开运算和闭运算如何与邻域像素作用的形状和尺寸模板，可以是矩形、椭圆、十字形或菱形。

**RMSE**：均方根误差（Root Mean Square Error），把预测图像与参考图像的像素差平方后求平均再开方，用于综合衡量误差大小；它不能单独代表边缘保真度或缺陷检出率。

### 延伸阅读

- 第34问的 Blob 分析直接依赖本题的预处理链，因为二值化质量往往决定连通域结果是否稳定。
- 第66问讨论现场环境光变化，可用于理解实验室调好的固定预处理链为何在产线部署后出现漂移。
- 第36问将本题的对比度与二值化处理延伸到 OCR、OCV 和条码读取。
...
<a id="q36"></a>
## 第36问：什么是 OCR 和 OCV？如何检测字符、条码、二维码的可读性和正确性？
---
...
![OCV 与条码质量评估的流程示意](images/q31-q40/part7_36_01_ocv_barcode_flow.png)


<p align="center"><strong>图36-1 OCV、OCR 与条码质量评估的典型处理链</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">
图36-1 把字符和条码项目中容易混淆的几个环节拆开了：左端是输入图像，中间上支路先做定位与角度校正，再进入 OCV/OCR；下支路经过对比度调整和二值化后，进入一维码或二维码解码；两条支路最终都汇入质量评分模块，用于检查缺笔、粘连、静区不足、对比度偏低等问题。工业现场还需要知道为什么这次能读、下次能否稳定读出，因此 OCV、OCR、解码和等级评估应被看成不同层面的任务。
</div>

#### 36.7.1 计算示例：把综合质量分数映射为印刷质量等级

本题采用的 0～4 分等级规则为：分数 ≥3.5 为 A，≥2.5 为 B，≥1.5 为 C，≥0.5 为 D，低于 0.5 为 F。边界必须按下限包含处理，例如 2.500 属于 B，0.500 属于 D，而不是相邻的低等级。

用四个仅用于演示的综合分数进行验证：样本 W 得分 3.80，对应 A；样本 X 得分 2.60，对应 B；样本 Y 得分 1.20，对应 D；样本 Z 得分 0.30，对应 F。样本 Z 仍可能在某次解码中成功，但它没有足够的质量余量。**“今天能读出来”和“过程质量达到可持续生产要求”是两项不同判断。**



![综合质量分数与印刷质量等级的映射示例](images/q31-q40/part7_36_02_grade_threshold_worked_example_zh.png)


<p align="center"><strong>图36-2 综合质量分数映射到 A～F 等级的示例</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">图36-2把阈值边界和四个示例分数放在一起。它说明质量等级是过程余量的记录方式，不等同于一次解码是否成功；具体评分项和验收阈值仍需按适用码制标准及客户要求确定。</div>
...
**把“成功解码”当成“印刷过程健康”。** 当前能读不代表在更快节拍、更远工作距离或更差照明下仍有稳定余量。

**图像质量没有确认，就先更换复杂解码算法。** 许多读码失败来自打印缺陷、失焦、反光和分辨率不足，照明、对焦和曝光应先于算法更换。

**对“完全读不到”和“读到但读错”使用同一套排查流程。** 前者多半首先检查成像与定位，后者还要检查分割、解码逻辑和内容校验。

**不看字符格式是否固定，就默认选择深度学习 OCR 或传统 OCV。** 格式高度固定时，轻量的模板或规则方法通常更容易维护；字符变化大、样本复杂时，深度学习方法才更有优势。

### 术语与符号表

**OCV**：光学字符验证，检查图像中的字符是否与预期字符串或参考模板一致，回答“内容是否正确”。

**OCR**：光学字符识别，从图像中读取字符内容，回答“图里写的是什么”。

**静区（quiet zone）**：条码或二维码周围必须保留的空白边界，静区不足会降低解码稳定性。

**印刷质量等级 A～F**：把对比度、调制度、固定图案损伤等多个质量维度合成为字母等级的过程指标，不等同于简单的读码成功或失败。

**点阵打刻**：通过冲针在金属等零件表面形成字符或二维码凹点的直接标记方式，通常需要利用方向性照明突出凹凸起伏。

**ROI**：感兴趣区域（Region of Interest），指字符、条码或二维码所在的有效图像范围。定位 ROI 后再识别，通常比在整幅图像中直接搜索更稳定。

**ISO/IEC 15415、15416**：分别用于二维符号和一维条码印刷质量评价的标准体系，具体验收仍应以项目适用版本和客户要求为准。

### 延伸阅读

- ISO/IEC 15415 用于二维符号印刷质量评价，ISO/IEC 15416 对应一维条码评价；正式验收时应以当前发布版本和客户附加要求为准。
- 第35问的图像预处理直接支撑本题中的对比度调整和二值化分支。
...
<a id="q37"></a>
## 第37问：什么是 MTF（调制传递函数）？如何用它评价镜头和相机系统的真实解像力？
---
...
![不同镜头的 MTF 曲线对比](images/q31-q40/part7_37_01_mtf_curves.png)


<p align="center"><strong>图37-1 不同成像质量条件下的 MTF 曲线对比</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">
图37-1横轴表示归一化空间频率，纵轴表示该频率下仍能保留多少亮暗调制度。蓝、橙、红三条曲线分别代表较高、中等和较低的成像质量水平；曲线下降越慢，说明系统在较高频率处仍能保留更多细节。图中虚线给出 MTF50 的常见观察位置，适合用于镜头、对焦和装调质量的横向比较，但正式验收仍需统一斜边法测试条件。
</div>

##### 37.2.1 计算示例：不同像元尺寸对应的奈奎斯特频率

由式（37-1）$f_N=1/(2p)$ 可直接计算像元采样的奈奎斯特频率。取三个仅用于演示的像元尺寸：$p=3.45\,\mu m$ 时，$f_N\approx144.93$ lp/mm；$p=2.40\,\mu m$ 时，$f_N\approx208.33$ lp/mm；$p=1.55\,\mu m$ 时，$f_N\approx322.58$ lp/mm。

**像元尺寸减半，传感器要求镜头在更高空间频率处仍保持有效调制度。** 这正是小像元系统更容易受到衍射和镜头分辨能力限制的原因。下图的三个像元尺寸只是公式演示值，不代表某一项目的选型建议。



![像元尺寸与奈奎斯特频率的关系](images/q31-q40/part7_37_02_nyquist_vs_pixel_pitch_zh.png)


<p align="center"><strong>图37-2 根据式（37-1）计算像元尺寸与奈奎斯特频率的关系</strong></p>
...
**把 MTF50 或 MTF20 当成统一的合格/不合格阈值。** 它们是便于比较的工程参考点，优先使用哪一个取决于任务更依赖高对比边缘还是接近极限的细节。

**只看镜头的 MTF 数值，不与传感器奈奎斯特频率比较。** 镜头曲线必须放到实际像元尺寸对应的 $f_N=1/(2p)$ 处判断，否则无法知道传感器采样能力是否真的被利用。

**把一个视场位置、一个光圈下的 MTF 测量当成整套系统的代表。** 中心与边缘、光圈、工作距离和测试条件都会改变曲线，单点数据不能替代完整评估。

**认为实验室 MTF 与现场图像不一致就说明 MTF 方法失效。** 现场失焦、装调偏心、振动、温漂和保护玻璃附加像差都可能造成差异，应先检查装调与工况。

### 术语与符号表

**MTF（调制传递函数）**：描述成像系统随空间频率变化保留多少对比度的曲线。

**奈奎斯特频率**：由像元采样决定、在不发生混叠时可表示的最高空间频率，$f_N=1/(2p)$。

**MTF50/MTF20**：调制度下降到 50% 和 20% 时对应的空间频率，是常用比较位置，不是绝对验收阈值。

**斜边法**：利用略微倾斜的高对比边缘，通过边缘扩散函数、线扩散函数和傅里叶变换测量系统 MTF 的方法。

**最佳工作光圈**：像差和衍射取得平衡、MTF 通常较高的中间 F 值范围，常见于 F4～F8，但具体值取决于镜头设计。

**lp/mm**：每毫米线对数（line pairs per millimeter），空间频率单位。一个线对由一条亮线和一条暗线组成。

**F 值**：光圈值，近似等于焦距与有效入瞳直径之比。F 值变大时通光量通常减少，景深可能增加，但衍射影响也会增强。

### 延伸阅读

第37.5节介绍的斜边法在 ISO 12233 中有系统化规定，该标准是电子成像设备分辨率和空间频率响应测量中常用的参考标准。正式测试时仍需统一靶标、照明、采样、像元尺寸、光圈和对焦条件。
...
<a id="q38"></a>
## 第38问：什么是无监督异常检测（Anomaly Detection）？在没有缺陷样本的情况下如何训练工业检测模型？
---
...
![无监督异常检测三类主流方法示意](images/q31-q40/part7_38_01_anomaly_method_map.png)
...
#### 38.6.1 计算示例：根据 PatchCore 风格的最近邻得分计算 AUROC

下面用一个合成的二维特征示例具体说明 AUROC 和 PatchCore 的最近邻打分机制。它不是 MVTec AD 数据集，也不是论文基准结果，只用于演示计算过程：正常特征聚集在 ((0,0)) 附近，异常特征聚集在 ((3,3)) 附近；从正常特征中取 200 个建立 Memory Bank，再准备 150 个正常测试样本和 150 个异常测试样本。

对每个测试特征，计算它到 Memory Bank 中最近特征的欧氏距离，并把这个距离作为异常分数。正常测试 patch 的平均最近邻距离约为 0.1528，异常测试 patch 约为 2.5241；按排名计算得到的 AUROC 约为 0.9982。若取两类均值的中点 1.3385 作为演示阈值，示例精确率为 1.0、召回率为 0.8267。这个阈值只用于展示混淆矩阵的含义，不能直接作为生产阈值。



![PatchCore 最近邻得分和 AUROC 计算示例](images/q31-q40/part7_38_02_worked_example_auroc_zh.png)


<p align="center"><strong>图38-2 合成二维特征空间中的最近邻异常得分与 AUROC 示例</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">图38-2把 PatchCore 的“正常特征库 + 最近邻距离”过程画成可观察的数值例子。正常测试点靠近 Memory Bank，异常点距离更远，因此 AUROC 较高；真实部署中的特征分布、阈值和类别难度都不同，不能把这个合成结果当成现场指标。</div>

#### 38.7 MVTec AD 数据集是什么？用它做基准测试时需要注意什么？

MVTec AD 是工业异常检测研究中常用的公开基准数据集，包含物体类和纹理类类别，提供正常训练样本与带异常的测试样本。它的价值在于给不同方法提供共同的比较入口，但公开数据集中的照明、采样方式和背景通常比真实产线更受控。

该数据集包含 15 个类别，其中 10 个为螺钉、药片、金属螺母等物体类别，5 个为皮革、地毯、木材等纹理类别；共有 5354 张无缺陷训练图像和 1725 张测试图像，覆盖 70 多种真实工业缺陷，并提供像素级标注。使用时应同时进行逐类别分析，因为不同类别的难度差异很大；**MVTec AD 上的高分不能直接等同于目标工厂的可部署性能。**

#### 38.8 在实际工业部署中，无监督异常检测通常如何与传统阈值方法结合使用？

常见做法是先由模型输出异常热图或图像级异常分数，再用阈值、Blob 分析、面积筛选和业务规则完成后处理。例如判断异常区域面积是否超过阈值、是否落在关键区域，或者是否与已知噪声模式重合。

这种组合把职责分开：学习模型负责发现“不像正常样本的区域”，规则层负责把原始信号转化为稳定、可解释、可维护的产线决策。

### 常见误区

**把重建误差变小直接理解为结果变好。** 强大的自编码器可能把某些异常也重建得很像，从而让异常误差变小，反而掩盖缺陷。

**只看 AUROC 就判断定位效果。** 图像级 AUROC 只衡量整幅图像的分离能力，不能说明异常像素是否定位准确；区域定位还需结合 PRO 等指标。

**认为 MVTec AD 的高分可以直接迁移到现场。** 公开数据集的背景、照明和采集条件通常更稳定，必须用目标产线自己的样本做验证。

**直接把原始异常分数当成最终结论。** 热图和分数通常还要经过阈值、连通域、面积和业务规则处理，才能形成可验收的判定。

### 术语与符号表

**无监督异常检测**：只用正常样本学习正常分布，再把偏离该分布的样本标记为异常，不依赖带标签的缺陷类别训练。

**Memory Bank**：PatchCore 从正常图像 patch 特征中建立并经核心集压缩的特征库，推理时用于最近邻异常打分。

**归一化流**：用可逆变换把复杂特征分布映射到已知简单分布（如高斯分布）的建模方法，FastFlow 等方法可据此估计异常概率。

**AUROC**：ROC 曲线下面积，衡量异常分数区分正常与异常样本的能力，与固定阈值无关，0.5 近似随机，1.0 表示理想区分。

**PRO**：区域重叠质量指标，分别计算每个异常连通区域被正确覆盖的比例，再对区域进行平均，更关注定位而不仅是整图分类。

**MVTec AD**：常用工业异常检测公开数据集，包含 15 个物体/纹理类别、5354 张无缺陷训练图像和 1725 张测试图像，并带有像素级缺陷标注。

**ResNet/WideResNet**：用于提取图像特征的卷积神经网络骨干。PatchCore 使用它们得到局部 patch 特征，再建立正常特征库；它们本身不是异常判定规则。

**patch**：图像中经过网络特征提取的局部区域表示。PatchCore 以 patch 级距离定位局部异常，再汇总为图像级分数。

### 延伸阅读

MVTec AD 数据集和 PatchCore、PaDiM、DRAEM、STFPM 等方法的开源实现可作为进一步阅读材料。正式研究或项目报告中，应同时记录数据划分、类别级结果和现场样本验证结果。
...
<a id="q39"></a>
## 第39问：什么是双目立体视觉的视差（Disparity）？如何从视差图计算深度？误差来源有哪些？
---
...
![双目几何、深度计算与误差趋势示意](images/q31-q40/part7_39_01_stereo_depth_error.png)
...
#### 39.6.1 计算示例：由视差计算深度，并验证误差随 Z² 增长

考虑一组经过立体矫正的双目相机，焦距 $f=1000$ 像素，基线 $B=0.10$ m。由 $Z=fB/d$ 可得：视差 $d=200$ px 时深度为 0.500 m，$d=100$ px 时为 1.000 m，$d=50$ px 时为 2.000 m，$d=40$ px 时为 2.500 m，$d=20$ px 时为 5.000 m。

再取亚像素视差噪声 $\sigma_d=0.2$ px，代入式（39-2）。在 $Z=1.0$ m 处，深度误差 $\sigma_Z=2.000$ mm；距离增加到 $Z=2.0$ m 后，误差变为 8.000 mm，恰好是 4 倍。若在 $Z=2.0$ m 处把基线加倍到 0.20 m，误差降为 4.000 mm。**距离加倍会使固定视差噪声造成的误差增加约 4 倍，基线加倍则可使远距离误差约减半。**



![视差计算深度与距离误差增长示例](images/q31-q40/part7_39_02_worked_example_depth_error_zh.png)


<p align="center"><strong>图39-2 由视差恢复深度，以及不同基线下深度误差随距离增长的示例</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">左图对应 $Z=fB/d$ 的深度换算，右图对应 $\sigma_Z\approx Z^2\sigma_d/(fB)$ 的误差传播。数值采用固定的示例焦距、基线和视差噪声，目的是显示平方增长关系；实际系统仍需用标定后的焦距、基线和匹配误差分布评估。</div>

### 常见误区

**在未经立体矫正的图像上直接使用 $Z=fB/d$。** 该公式假设左右极线已经对齐、使用共享焦距；直接把原始图像的视差代入会得到错误深度。

**认为双目精度在整个工作范围内近似不变。** 固定视差噪声下，深度误差随 $Z^2$ 增长，1 m 验证良好不代表 2 m 仍只有略微变差。

**只为远距离精度增大基线，不检查近距离后果。** 基线变大通常会增加遮挡、缩小近距离可见范围并提高安装与标定难度。

**期望整幅图像都能得到同样可靠的稠密视差。** 无纹理区域、遮挡边界、镜面和透明表面本来就是双目匹配的薄弱区域。

### 术语与符号表

**视差 (d)**：同一空间点在左右立体矫正图像中的水平位置差，式（39-1）用它换算深度。

**基线 (B)**：左右相机光心之间的物理距离。基线越大，远距离视差通常越大，但近距离遮挡也更明显。

**极线约束**：同一空间点在另一幅图像中的对应点只能落在特定极线上，而不是整幅图像任意位置；矫正后该极线可变成水平图像行。

**立体矫正**：通过标定得到的图像变换，使左右极线对齐到共同的水平行，并满足深度公式的几何假设。

**SGM/SGBM**：半全局匹配及其块匹配变体，用于在精度、速度和实现复杂度之间折中，计算稠密视差图。

**深度误差传播 $\sigma_Z$**：$\sigma_Z\approx Z^2\sigma_d/(fB)$，描述固定视差误差如何随距离转换为深度不确定度。

**OpenCV**：开源计算机视觉软件库，提供双目标定、立体矫正、重映射和视差计算等函数。函数调用需要与相机标定参数、图像尺寸和坐标约定保持一致。

**px**：像素（pixel）的缩写。公式中的焦距 (f) 若以 px 表示，基线 (B) 和深度 (Z) 应使用相互一致的长度单位。

### 延伸阅读

可继续查阅 OpenCV 的相机标定与三维重建文档（stereoCalibrate、stereoRectify、remap）以及双目深度图教程。正式项目应把视差误差、遮挡区域、工作距离和验收容差一起纳入测试。
...
<a id="q40"></a>
## 第40问：视觉系统项目验收时，客户通常提出哪些标准？如何设计完整的量产前验收测试方案？
---
...
样件除了标准好样件和明显坏样件，更重要的是覆盖临界样件，也就是最容易引发争议的边界件，例如接近公差上限、缺陷刚好接近阈值、反光最重、姿态最不稳的一类样本。
...
![视觉系统验收测试流程与样件分层策略](images/q31-q40/part7_40_01_acceptance_workflow.png)
...
#### 40.3.1 计算示例：ANOVA GR&R 的方差分解

下面用一个仅用于演示的 GR&R 研究说明方差分解过程：5 个具有代表性的样件、3 名操作员、每名操作员对每个样件重复测量 2 次，共 30 次关键尺寸测量。示例计算得到：重复性 EV 占总变异的 9.2%，再现性 AV 占 11.8%，合并 GR&R 占 15.0%，零件间变异 PV 占 98.9%，NDC 为 9。

按工程中常见的 AIAG 风格参考区间，GR&R 小于 10% 通常可接受，10%～30% 需要结合用途判断，大于 30% 通常不可接受。这个演示结果 15.0% 属于条件可接受区间，且操作员间再现性略大于重复性，排查重点应放在夹具、操作流程和定位一致性，而不是先怀疑相机或算法。这里的样件数量和阈值用于解释方法，不构成固定验收方案。



![GR&R 方差分解计算示例](images/q31-q40/part7_40_02_grr_worked_example_zh.png)


<p align="center"><strong>图40-2 GR&R 方差分解示例（演示数据）</strong></p>
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">图40-2把总变异拆成重复性、再现性和零件间变异。GR&R 不是一个孤立的准确率数字：它还要回答误差主要来自设备重复测量，还是来自操作员、装夹和条件变化。图中数字属于固定随机种子的合成示例，正式验收应使用项目真实样件和事先约定的 MSA 口径。</div>

### 常见误区

**到验收当天才决定通过标准。** 指标、阈值、样件和判定规则应在验收前冻结，否则技术评审很容易变成没有参照文件的现场协商。

**只给一个准确率数字就认为测量系统已经被证明。** 单一准确率无法区分重复性和再现性，GR&R 的分解才能告诉工程师应该检查设备、夹具还是操作流程。

**只准备明显合格件和明显不合格件，跳过临界样件。** 容差边界、检出阈值、强反光和姿态不稳定的样件，往往最容易引发客户与供应商争议，也最容易被样本计划漏掉。

**只做短时间演示运行。** 对焦漂移、通信超时、光源热衰减和日志累积等问题往往要经过长时间运行才会出现，短演示不能证明量产稳定性。

### 术语与符号表

**验收测试**：在客户现场、真实节拍、真实样件和真实运行条件下，对视觉系统是否具备上线条件进行的正式评价，与实验室可行性测试不同。

**GR&R**：量具重复性与再现性分析，把测量系统总变异分解为设备重复性和操作员/条件变化造成的再现性。

**漏检率/误检率**：缺陷品被判为合格，以及合格品被判为缺陷的比例，是检测和分拣项目的核心错误指标。

**黄金样件标准**：验收前预先约定的合格品与缺陷品判定参考，用来让争议可以回到共同证据上。

**分层样件集**：按正常件、典型缺陷件、临界件和特殊工况件分层，并在测试前冻结数量和比例的验收样本池。

**NDC**：可区分类别数，估计测量系统在零件间变异范围内能够可靠区分多少个不同水平。

**ANOVA**：方差分析（Analysis of Variance），把观测值的总变异分解为不同来源，用于 GR&R 研究中的变异归因。

**MSA**：测量系统分析（Measurement Systems Analysis），系统评价测量设备、人员、方法和环境造成的测量变异。

**AIAG**：汽车工业行动集团（Automotive Industry Action Group），其 MSA 手册是工业测量系统分析中常用的参考资料之一。

### 延伸阅读

AIAG 的 Measurement Systems Analysis（MSA）手册是 ANOVA GR&R 和常见变异接受区间的重要行业参考。ISO 9001 相关质量管理实践则可用于补充验收指标、记录追溯和版本管理要求。
