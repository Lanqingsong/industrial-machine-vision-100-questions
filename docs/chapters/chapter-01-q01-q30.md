...
**平衡策略：**
分辨率与帧率的平衡，体现为单帧信息量与单位时间吞吐量之间的取舍；像元尺寸与分辨率的平衡，则是在收光能力和空间采样能力之间分配传感器面积。工程上可通过选择合适的接口（如GigE、USB3、CoaXPress）、优化照明和镜头系统、拆分检测工位或缩小视场来降低单个参数的压力。
...
公式(2-1)/(2-2)中的 k 直接影响下游视觉算法提取目标的稳定性，不能只凭经验设定。下面用一段可离线复现的 OpenCV 参考代码，固定视场和固定传感器噪声水平，让同一枚0.1mm焊点缺陷的像素直径分别等于 k=1,2,3,4（对应表2-1中从"仅测量口径"到"精密零件测量"档的跨度），然后用 Canny 边缘检测尝试提取其轮廓：
...
<div class="figure-note" style="background: transparent; border: 0; padding: 0; margin: 0.35em 0 1.2em;">
图2-2 固定16×16像素画幅、固定传感器噪声水平（σ=9 DN），同一枚0.1mm焊点缺陷分别按k=1、2、3、4配置相机分辨率。结果显示：k=1（仅测量口径）和k=2（表2-1中"一般外观检查"档，本不是为这种低对比小缺陷设计的）都未能在中心区域检出有效轮廓；k=3（表2-1推荐的PCB焊点检测档）开始，缺陷作为多像素连通结构可以稳定地与噪声区分。这组结果说明，不同应用类型需要不同的 k 值，原因是缺陷尺寸、对比度和噪声容忍度不同，**跨场景直接套用别人项目里的k值是本节最需要提防的误区之一**（详见§2.10）。
</div>
...
这个结果给出的是由光子散粒噪声决定的**理论上限**。它说明，在理想条件下，信号电子数增加 4 倍，SNR 只能提高 2 倍。提高光照、延长曝光和提高量子效率都用于积累更多有效电子，而收益遵循平方根规律。
...
它的价值并不只是像素多。线阵系统通常便于与编码器、线光源和节拍控制协同，在连续生产线上可以把采样间距锁定到较稳定的物理步长。只要运动均匀且同步可靠，纵向比例关系也更容易保持一致。线阵的优势来自传感器与连续运动工艺的匹配；匹配建立后，宽幅、高速、全检这三个通常彼此矛盾的要求，才有可能在同一方案中同时实现。
...
这里 $t_{\mathrm{exp}}$ 为曝光时间，$t_{\mathrm{read}}$ 为单行读出及相关死区时间。行频一端受工艺速度驱动，另一端受相机、接口和照明条件共同限制。如果计算得到的需求值高于系统可提供的稳定行频，说明整套方案已经逼近或超过物理边界，需要重新审视工艺速度、行频余量和照明配置。
...
方案能否落地，最终取决于样件验证。相同材质名称下，粗糙度、镀层、油膜、保护膜、加工纹理和环境反射都会改变结果。工程上最稳妥的做法，是围绕目标缺陷制作一组代表性样件，在固定镜头、工作距离和节拍条件下，逐项比较图像对比度与稳定性，而不是只凭经验标签做判断。
...
光源颜色反映的是照明波长分布的差别。不同波长与材料相互作用时，在反射、吸收、透射、散射、荧光激发和传感器响应上的表现并不相同，因此最终影响图像对比。为视觉系统选颜色，应先明确需要哪一部分变亮、哪一部分变暗，以及这种差异来自哪一种物理机制，而非仅问工件是什么颜色。
...
结论不能简化为二选一口号：单节点、固定任务、追求快速落地时，智能相机常更省心；多工位、复杂逻辑、需要持续扩展时，工控机架构通常更有长期优势。
...
未来更有价值的是把散热设计前置，并与安装结构和长期可靠性绑定，而不是追逐某一种“新材料”名称。
...
光学防抖通过检测抖动，再移动镜片组或传感器来抵消部分画面位移，使图像投影尽量稳定在传感器上。与之相近的电子防抖，则多通过裁剪、重采样和时序补偿来实现更平稳的视频观感。两者主要用于改善观看体验或宏观识别效果，不能替代严格固定的相机内参模型。
