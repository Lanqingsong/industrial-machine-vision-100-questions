...
若视觉端将OK/NG脉冲设为20 ms，名义裕量为 (20-19=1\ \mathrm{ms})。这些时间均为示例值，用于说明计算方法，不代表现场参数。实际设计还要考虑输入模块响应时间、扫描相位、通信任务周期和信号抖动，不能只依据一个理想公式设定脉冲。
...
若从PLC一侧表述，术语就应当更严谨一些。PLC用于接收I/O结果的是数字量输入模块，用于接收网络结果的是通信模块、总线接口模块或CPU自带的工业以太网接口；PLC真正的输出模块，则多用于驱动电磁阀、继电器、蜂鸣器、指示灯或剔除执行器。区分这两个方向，有助于后续理解接线、时序和故障诊断中的信号流向。
...
隔离应与合理接地、屏蔽线与屏蔽层正确处理、感性负载吸收回路、输入滤波、浪涌保护以及恰当的公共端规划配合使用。隔离是系统抗干扰设计的一部分，不能替代布线和接地上的基本措施。
...
不过，自定义Socket虽然灵活，却把协议定义、字段解释、异常处理、重连策略和版本兼容责任更多留给了集成方。若项目后续维护人员不足，或者跨厂商协同频繁，这种自由度很高的方式未必最省心。能够发出数据并不等于后期容易维护，协议文档、异常处理和版本兼容仍需由集成方承担。
...
两者的主要差异在于承载介质与网络组织形式。RTU更适合总线式串行环境，硬件简单、历史设备多；TCP更适合现代以太网环境，布网、扩展和上位集成都更方便。Modbus TCP适合结果交换，但不应直接视为实时网络协议。如果项目对控制周期抖动和确定性要求很高，仍需结合控制器能力和网络实测来判断是否足够。
...
选型应先界定本项目的数据路径、控制路径和信息路径，再判断每一段链路适合使用什么技术，不能在一串协议名里直接寻找“最强”的方案。
...
#### 49.3 地址号为什么经常让初学者出错？
一个常见误区，是把资料中的“40001、30001、00001”这类表示法，当成设备内部真实地址。很多手册里的前缀编号只是为了帮助区分对象类型，通信帧里使用的往往是从零开始或按厂商定义偏移的地址。因此，手册写“40001”并不必然意味着程序里就要直接写40001。
...
“网络拓扑”和“协议机理”需要分开理解。星形、树形、线形、环形描述的是布线和网络连接方式；IRT、DC、周期调度和帧内处理描述的是实时通信机理。某一种拓扑不能直接等同于某一种协议性能。
...
按测量对象区分，常见有旋转编码器和直线编码器；按输出方式区分，常见有增量式和绝对式。两个分类维度需要分开，不能把“旋转式”和“增量式”当成同义词。
...
线阵系统更像“持续按位移采样”，面阵系统更像“在关键位置采样整帧”。二者都需要速度同步，但同步目标并不完全相同。
